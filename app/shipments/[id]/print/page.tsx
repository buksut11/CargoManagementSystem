...
export default function PrintShipmentPage() {
  ...
  if (!shipment) {
    return <p className="p-8 text-sm text-slate-400">{t("Loading receipt…")}</p>;
  }
  ...
  const rows: [string, string][] = [
    ...
    ["Status", t(STATUS_LABEL[shipment.status])],
    ...
    <div className="print-sheet mx-auto max-w-2xl bg-white p-8 text-slate-900 print:p-0">
      ...
      <div className="border border-slate-300 p-10">
        ...
        <div className="mt-6 flex justify-end text-right">
          <div>
            ...
            <div className="mt-1 text-sm text-slate-500">
              {t("Created {date}", { date: fmtDate(shipment.created_at) })}
            </div>
          </div>
        </div>
        ...
        <div className="mt-14 grid grid-cols-2 gap-10 text-sm text-slate-500">
          <div className="border-t border-slate-400 pt-2">{t("Sender signature")}</div>

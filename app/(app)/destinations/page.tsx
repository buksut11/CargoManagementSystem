...
export default function DestinationsPage() {
  ...
  async function save(e: React.FormEvent) {
    ...
    if (error) {
      setError(
        error.code === "23505"
          ? t("That destination already exists.")
          : error.message,
      );
      return;
    }
  ...
  return (
    <div>
      ...
      <Section
        icon={<PinIcon />}
        title={editingId ? t("Edit destination") : t("Add a destination")}
        subtitle={t("Places you ship to")}
      >
        ...
        <form onSubmit={save} className="flex flex-wrap items-end gap-3">
          ...
          <div className="min-w-32 flex-1">
            ...
            <Input
              ...
              placeholder={t("e.g. Türkiye")}
            />
          </div>
          <div className="flex gap-2">
            <Button type="submit" disabled={busy}>
              {busy ? t("Saving…") : editingId ? t("Save changes") : t("Add")}
            </Button>
            {editingId && (
      ...
      <Card className="overflow-hidden">
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-0.5 flex items-center gap-1 truncate text-xs text-slate-500 dark:text-slate-400">
                    ...
                    <span className="truncate">{d.country || t("No country")}</span>
                  </div>
                </div>
        ...
        {!loading && destinations.length === 0 && (
          <EmptyState message={t("No destinations yet — add the places you ship to.")} />
        )}
      </Card>
      </div>
      ...
      <ConfirmDialog
        ...
        message={
          pending
            ? t('Delete "{name}"? Shipments using it will show no destination.', {
                name: pending.name,
              })
            : undefined
        }

...
// date on the wrong day). Matches how invoice/paid dates are stored.
...
export default function CargoStatementPage() {
    ...
    async function load() {
      ...
      if (ids.length) {
        const [s, p] = await Promise.all([
          ...
          supabase
            .from("payments")
            .select("invoice_id, amount, paid_date, method")
            .in("invoice_id", ids),
        ]);
        shipments = (s.data as ShipmentRow[]) ?? [];
      ...
      for (const sh of shipments) {
        ...
        const list = shipByInvoice.get(sh.invoice_id) ?? [];
        list.push(sh);
        shipByInvoice.set(sh.invoice_id, list);
      }
      const destName = (sh: ShipmentRow): string => {
      ...
      const rows: Line[] = [
        ...invoices.map((iv) => {
          ...
          const manifest = ships
            ...
            .filter(Boolean)
            .join(", ");
          const description = ["Cargo invoice", dests].filter(Boolean).join(" · ");
          return {
            date: iv.issued_date,
            ...
            <div className="flex flex-col items-center gap-1 text-center">
              ...
              {!orgHeader?.logo_url && (
                <div className="text-2xl font-bold leading-tight">
                  {orgHeader?.name ?? org?.orgName ?? "📦 CargoBook"}
                </div>
              )}
              {orgHeader?.address && (
